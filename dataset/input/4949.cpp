...
int main() {
    ...
    // For consistent testing of the logical error, the secret number is fixed.
    // In a correct implementation, this would be `int secretNumber = (rand() % 100) + 1;`
    int secretNumber = 42; // Fixed secret number for predictable test cases
    ...
    while (attempts < MAX_ATTEMPTS) {
        ...
        else if (guess < secretNumber) { // If guess is lower than secret
            std::cout << "Too high! Try again.\n"; // Should be "Too low!"
        } else { // If guess is higher than secret
