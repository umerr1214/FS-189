...
public:
    ...
    // Copy assignment operator - Correct Version (Copy-and-Swap Idiom)
    MyString& operator=(const MyString& other) {
        if (this == &other) { // Handle self-assignment
            return *this;
        }

        // 1. Create a temporary object using the copy constructor.
        // This handles allocation, copying, and potential exceptions safely.
        // If 'temp' construction fails, 'this' object remains unchanged (strong exception guarantee).
        MyString temp(other);

        // 2. Swap the internal resources (data_ and size_) with the temporary object.
        // After the swap, 'this' now holds the new data from 'other',
        // and 'temp' holds the *original* data of 'this'.
        std::swap(data_, temp.data_);
        std::swap(size_, temp.size_);

        // 3. When 'temp' goes out of scope, its destructor will automatically
        // clean up the old data that 'this' used to own.
        return *this;
    }
...
int main() {
    std::cout << "--- Testing MyString Copy Assignment Operator (Correct Version) ---" << std::endl;
