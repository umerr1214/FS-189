...
public:
    ...
    // SEMANTIC ERROR: Returns a reference to a local temporary object
    Fraction& operator*(const Fraction& other) const {
        int new_num = numerator * other.numerator;
        int new_den = denominator * other.denominator;
        Fraction result(new_num, new_den); // 'result' is a local variable
        return result; // Returning a reference to a stack-allocated 'result' which will be destroyed
    }
...
int main() {
    ...
    Fraction f1(1, 2);
    Fraction f2(3, 4);
    Fraction f3 = f1 * f2; // Undefined behavior: f3 receives a dangling reference
    std::cout << f1 << " * " << f2 << " = " << f3 << std::endl;

    Fraction f4(2, 3);
    Fraction f5(3, 2);
    Fraction f6 = f4 * f5; // Undefined behavior
    std::cout << f4 << " * " << f5 << " = " << f6 << std::endl;

    Fraction f7(-1, 2);
    Fraction f8(1, -2);
    Fraction f9 = f7 * f8; // Undefined behavior
    std::cout << f7 << " * " << f8 << " = " << f9 << std::endl;

    Fraction f10(0, 5);
    Fraction f11(7, 8);
    Fraction f12 = f10 * f11; // Undefined behavior
