...
public:
    ...
    void addBook(const Book& book) {
        // SEMANTIC ERROR: No check for 'numBooks < capacity'.
        // This will lead to an out-of-bounds write (undefined behavior)
        // when trying to add more books than the allocated 'capacity'.
        books[numBooks++] = book;
        if (numBooks > capacity) { // This check is too late, assignment already happened
            std::cout << "Library is full. Cannot add more books." << std::endl;
            // Optionally, decrement numBooks to revert, but the damage is done.
            // For this example, we let the out-of-bounds write occur.
        }
    }
...
int main() {
    ...
    myLibrary.addBook(Book("Neuromancer", "William Gibson", 1984)); // This add will cause out-of-bounds write
