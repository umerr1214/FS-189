...
int main() {
    ...
    Student s1;
    // Semantic Error: Attempting to access private member 'name' directly from main
    std::cout << "Student 1 (Default): Name=" << s1.name << ", ID=" << s1.getId() << ", GPA=" << std::fixed << std::setprecision(1) << s1.getGpa() << ", Eligible=" << (s1.isEligibleForScholarship() ? "true" : "false") << std::endl;

    // Test case 2: Parameterized constructor (eligible)
    ...
    // Test case 3: Parameterized constructor (not eligible)
    ...
    // Test case 4: Parameterized constructor (borderline eligible)
    ...
    // Test case 5: Parameterized constructor (borderline not eligible)
