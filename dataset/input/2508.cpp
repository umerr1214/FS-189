...
public:
    ...
    // Overload == operator with semantic error: missing const qualifier
    // This allows compilation and correct behavior for non-const objects,
    // but semantically, operator== should not modify the object and should be callable on const objects.
    bool operator==(const Product& other) { // SEMANTIC ERROR: Should be 'const'
