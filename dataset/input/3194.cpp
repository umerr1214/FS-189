...
public:
    ...
    // Copy assignment operator - Readability / Efficiency Issue
    // This version is correct and robust (strong exception guarantee),
    // but uses a manual loop for string copying instead of `strcpy`,
    // which is less readable and potentially less efficient (compiler might optimize `strcpy` better).
    MyString& operator=(const MyString& other) {
        if (this == &other) {
            return *this;
        }

        // Allocate new memory first to ensure strong exception guarantee.
        // If this allocation fails, the original object remains unchanged.
        char* new_data = new char[other.size_ + 1];
        size_t new_size = other.size_;

        // Manual loop for copying - less readable and potentially less efficient than strcpy
        for (size_t i = 0; i <= new_size; ++i) { // Copy null terminator too
            new_data[i] = other.data_[i];
        }

        // If allocation and copy succeeded, then deallocate old data and update members
        delete[] data_;
        data_ = new_data;
        size_ = new_size;
...
int main() {
    std::cout << "--- Testing MyString Copy Assignment Operator (Readability/Efficiency Issue) ---" << std::endl;
