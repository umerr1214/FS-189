...
int* findMaxPointer(int* arr, int size) {
    // Handle edge cases: empty array or invalid pointer
    if (size <= 0 || arr == nullptr) {
        return nullptr;
    }

    // Initialize maxPtr to point to the first element.
    // This is safe because `size` is guaranteed to be > 0 and `arr` is not nullptr.
    int* maxPtr = arr;

    // Iterate through the rest of the array using pointer arithmetic.
    // Start from the second element (arr + 1) to compare with the first.
    for (int i = 1; i < size; ++i) {
        // Compare the value at the current position (`*(arr + i)`)
        // with the value pointed to by `maxPtr`.
        if (*(arr + i) > *maxPtr) {
...
int main() {
    ...
    std::cout << "Test Case 8: Null Pointer Array, Size 0" << std::endl;
    std::cout << "Expected: nullptr" << std::endl;
    std::cout << "Actual: " << run_test_and_get_output("null:0", findMaxPointer) << std::endl << std::endl;
