...
class Shape {
public:
    // ERROR: Destructor is NOT virtual, leading to logical error in polymorphic deletion
    ~Shape() {
...
public:
    ...
    ~Rectangle() override { // 'override' here indicates intent, but base is not virtual
...
public:
    ...
    ~Circle() override { // 'override' here indicates intent, but base is not virtual
...
void demonstratePolymorphism() {
    ...
    for (Shape* shape : shapes) {
        delete shape; // This will only call Shape's destructor, not derived destructors
    }
    shapes.clear();
}
