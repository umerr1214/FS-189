...
public:
    ...
    // LOGICAL ERROR: Incorrect denominator calculation
    Fraction operator*(const Fraction& other) const {
        int new_num = numerator * other.numerator;
        // The denominator is incorrectly added instead of multiplied
        int new_den = denominator + other.denominator; // Should be denominator * other.denominator
...
int main() {
    ...
    Fraction f1(1, 2);
    Fraction f2(3, 4);
    Fraction f3 = f1 * f2; // Expected: 3/8, Actual: 3/6 (simplified to 1/2)
    std::cout << f1 << " * " << f2 << " = " << f3 << std::endl;

    Fraction f4(2, 3);
    Fraction f5(3, 2);
    Fraction f6 = f4 * f5; // Expected: 1/1, Actual: 6/5
    std::cout << f4 << " * " << f5 << " = " << f6 << std::endl;

    Fraction f7(-1, 2);
    Fraction f8(1, -2); // Denominator becomes positive 2 in constructor
    Fraction f9 = f7 * f8; // Expected: 1/4, Actual: -1/0 (denominator set to 1, result -1/1)
    std::cout << f7 << " * " << f8 << " = " << f9 << std::endl;

    Fraction f10(0, 5);
    Fraction f11(7, 8);
    Fraction f12 = f10 * f11; // Expected: 0/1, Actual: 0/13 (simplified to 0/1)
