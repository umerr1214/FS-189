...
public:
    ...
    // READABILITY / EFFICIENCY ISSUE: This implementation returns `Counter` by value,
    // causing an unnecessary copy of the object. The idiomatic and more efficient
    // way is to return `Counter&` (a reference).
    Counter operator++() { // Returns by value
        ++count;
        return *this; // Creates a temporary copy
    }

    // Postfix increment operator (correct for this version)
    // Increments the count and returns a copy of the object's state *before* the increment.
    Counter operator++(int) {
        Counter temp = *this; // Save current state
        ++count;              // Increment internal count
...
int main() {
    ...
    Counter ref_c1 = ++c1; // Will be a copy, not a reference
    std::cout << "After ++c1: " << c1.getCount() << "\n";
    std::cout << "Ref value: " << ref_c1.getCount() << "\n"; // This 'ref_c1' is a copy, not a true reference
    ...
    Counter ref_c3 = ++(++c3); // Will involve multiple copies
    std::cout << "After ++(++c3): " << c3.getCount() << "\n";
    std::cout << "Ref value: " << ref_c3.getCount() << "\n"; // This 'ref_c3' is a copy
