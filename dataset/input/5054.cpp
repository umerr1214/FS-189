...
// Function template with readability/efficiency issues:
// Uses complex bitwise operations for signed integers and a verbose conditional for floating-point types,
// making it less readable and potentially less efficient than std::abs.
template <typename T>
T getAbsoluteValue(T val) {
    if constexpr (std::is_integral_v<T> && std::is_signed_v<T>) {
        // Bitwise trick for signed integers - less readable, potentially less efficient than std::abs
        // This calculates (val ^ mask) - mask where mask is all 1s if val < 0, all 0s if val >= 0
        T mask = val >> (sizeof(T) * 8 - 1); // Arithmetic right shift copies sign bit
        return (val + mask) ^ mask; // Equivalent to val = (val ^ mask) - mask;
    } else if constexpr (std::is_floating_point_v<T>) {
        // Overly verbose conditional for floating points - correct but not concise
        return val < static_cast<T>(0.0) ? static_cast<T>(0.0) - val : val;
    } else { // For unsigned types or other numeric types where abs is trivial (e.g., unsigned integers)
        return val; // Absolute value of unsigned is itself
    }
}
