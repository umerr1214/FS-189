...
public:
    ...
    // Delegate start method to the Engine - LOGICAL ERROR HERE
    void start() {
        std::cout << make << " " << model << " is attempting to start." << std::endl;
        engine.stop(); // Should call engine.start()
    }

    // Delegate stop method to the Engine - LOGICAL ERROR HERE
    void stop() {
        std::cout << make << " " << model << " is attempting to stop." << std::endl;
        engine.start(); // Should call engine.stop()
    }
};
