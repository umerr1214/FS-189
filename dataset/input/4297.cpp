...
// Function with a robustness issue: incorrectly handles empty strings.
// An empty string is generally considered a palindrome, but this implementation returns false.
bool is_palindrome(const std::string& s) {
    if (s.empty()) {
        return false; // Robustness issue: should be true for empty string
    }
...
int main() {
    ...
    std::cout << "Testing is_palindrome function (Robustness Issue: Empty string handling)\n";

    run_test("", true, passed_count, total_count); // Expected: true, but implementation returns false
