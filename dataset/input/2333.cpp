...
public:
    ...
    std::string getTitle() const { // Needed for the logical error
        return title;
    }
...
bool operator==(const Book& lhs, const Book& rhs) {
    return lhs.getTitle() == rhs.getTitle(); // Should be lhs.getIsbn() == rhs.getIsbn()
}
...
int main() {
    ...
    for (size_t i = 0; i < testCases.size(); ++i) {
        ...
        std::cout << "  Expected: " << (tc.second ? "true" : "false") << ", Got: " << (result ? "true" : "false") << std::endl;
        if (result == tc.second) {
