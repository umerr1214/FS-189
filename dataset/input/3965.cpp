...
void analyzeArray(int arr[], int size, int& minVal, int& maxVal) {
    ...
    // Logical Error: Initializing minVal and maxVal to 0 instead of arr[0]
    // This will cause incorrect results if all array elements are positive (min becomes 0)
    // or if all array elements are negative (max becomes 0).
    minVal = 0; 
    maxVal = 0;

    for (int i = 0; i < size; ++i) { // Loop starts from 0, which is fine, but initialization is the issue
        if (arr[i] < minVal) {
            minVal = arr[i];
        }
        if (arr[i] > maxVal) {
